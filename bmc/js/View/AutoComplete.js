...
export default class AutoComplete {
  ...
  _registerEvt() {
    ...
    $on(
      qs(".search_bar_in"),
      "input",
      this._renderAutoCompleteHandler.bind(this)
    );
    ...
    $on(
      ...
      this.storage.render.bind(this.storage, {
        key: "recent",
        targetNode: qs(".search_auto_list"),
        tpl: this.recentDataTpl,
        listLength: 5
      })
    );
    $on(
      qs(".search_icon"),
      "click",
      this.storage.save.bind(this.storage, {
        key: "recent",
        targetNode: qs(".search_bar_in")
      })
    );
  }
  ...
  async _renderAutoCompleteHandler(e) {
    try {
      const response = await fetch(`${this.apiUrl}/${e.target.value}.json`);
      const autoData = await response.text();
      const autoCompleteTpl = pipe(
        JSON.parse,
        this._strongSameLiteral,
        this.autoCompleteItemTpl
      )(autoData);
  ...
  upHighlight() {
    ...
    qsa(".highlight")[qsa(".highlight").length - 1].classList.remove(
      "highlight"
    );
  }
